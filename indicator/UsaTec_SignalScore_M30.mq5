//+------------------------------------------------------------------+
//|  UsaTec_SignalScore_M30.mq5  —  v2.0                             |
//|  Score composto: −1 (sell) a +1 (buy)                            |
//|  Aplicar no gráfico M30  —  Tendência via H4                     |
//+------------------------------------------------------------------+
...
//  PARAMETROS
//==================================================================
input group "=== M30 Indicadores ==="
input int    InpEMA9       = 9;
input int    InpEMA21      = 21;
input int    InpRSIPer     = 14;
input int    InpATRPer     = 14;

input group "=== H4 Filtros (Tendencia) ==="
input int    InpH4EMA21    = 21;
input int    InpH4EMA50    = 50;
input int    InpH4RSIPer   = 14;
input double InpRSILong    = 60.0;   // RSI H4 minimo para bull
input double InpRSIShort   = 40.0;   // RSI H4 maximo para bear

input group "=== RSI M30 Zonas ==="
...
int OnInit()
{
   ...
   // Handles
   string s = Symbol();
   hE9     = iMA(s, PERIOD_M30, InpEMA9, 0, MODE_EMA, PRICE_CLOSE);
   hE21    = iMA(s, PERIOD_M30, InpEMA21, 0, MODE_EMA, PRICE_CLOSE);
   hE50    = iMA(s, PERIOD_M30, 50, 0, MODE_EMA, PRICE_CLOSE);
   hRSI    = iRSI(s, PERIOD_M30, InpRSIPer, PRICE_CLOSE);
   hATR    = iATR(s, PERIOD_M30, InpATRPer);
   hH4E21  = iMA(s, PERIOD_H4, InpH4EMA21, 0, MODE_EMA, PRICE_CLOSE);
   hH4E50  = iMA(s, PERIOD_H4, InpH4EMA50, 0, MODE_EMA, PRICE_CLOSE);
   hH4RSI  = iRSI(s, PERIOD_H4, InpH4RSIPer, PRICE_CLOSE);

   if(hE9==INVALID_HANDLE||hE21==INVALID_HANDLE||hRSI==INVALID_HANDLE||
      hATR==INVALID_HANDLE||hH4E21==INVALID_HANDLE||hH4E50==INVALID_HANDLE||
      hH4RSI==INVALID_HANDLE)
   { Alert("Erro handles! ", GetLastError()); return INIT_FAILED; }

   // Nome e niveis
   string sname = StringFormat("UsaTec Score M30 [Thr:%.0f%%]", InpThresh*100);
...
{
   int handles[] = { hE9,hE21,hE50,hRSI,hATR,hH4E21,hH4E50,hH4RSI };
                ...
                const int &spread[])
{
   ...
   // ── Carregar H4 ────────────────────────────────────────────────
   // Limitar a quantidade necessaria: cada barra H4 = 8 barras M30
   ...
   ArraySetAsSeries(h4e21,  true);
   ArraySetAsSeries(h4e50,  true);
   ArraySetAsSeries(h4rsi,  true);
   ...
   // ── Carregar M30 (como serie: indice 0 = mais recente) ─────────
   ...
   if(CopyBuffer(hE9,  0, 0, rates_total, ae9)  < rates_total) return prev_calculated;
   if(CopyBuffer(hE21, 0, 0, rates_total, ae21) < rates_total) return prev_calculated;
   if(CopyBuffer(hE50, 0, 0, rates_total, ae50) < rates_total) return prev_calculated;
   if(CopyBuffer(hRSI, 0, 0, rates_total, arsi) < rates_total) return prev_calculated;
   if(CopyBuffer(hATR, 0, 0, rates_total, aatr) < rates_total) return prev_calculated;
   ...
   for(int i=start; i<rates_total; i++)
   {
      ...
      // Indicadores M30 da barra anterior (si1 no array serie)
      double e9v   = ae9 [si1];
      double e21v  = ae21[si1];
      double e21p  = ae21[si2];
      ...
      bool trend_bear = he21v < he50v;
      bool mom_bull   = hrsiv >= InpRSILong;
      bool mom_bear   = hrsiv <= InpRSIShort;
      ...
      double c2l = 0.0;
      if(mom_bull) c2l = W2*MathMin(1.0, 0.5+(hrsiv-InpRSILong)/(100.0-InpRSILong));
      ...
      double c2s = 0.0;
      if(mom_bear) c2s = W2*MathMin(1.0, 0.5+(InpRSIShort-hrsiv)/(InpRSIShort+0.001));
      ...
      if(i == rates_total-1)
      {
         pHe21=he21v; pHe50=he50v; pHrsi=hrsiv;
   ...
   // Atualizar painel
   if(InpPanel && panelOk)
      UpdatePanel(pHe21,pHe50,pHrsi,pE9,pE21,pCl,pOp,pRsi,pVwap,pNet,pFL,pFS);
...
void CreatePanel()
{
   ...
   MkLbl(PFX+"T0", "▌ UsaTec Score M30", panelWin, x,    y,    13, clrDodgerBlue);
   MkLbl(PFX+"T1", "─────────────────────", panelWin, x,    y+17, 9,  C'38,58,78');

   string lbl[7] = {
      "① Tend. H4  (EMA21>EMA50)",
      "② RSI H4   (≥60 bull / ≤40 bear)",
      "③ Pullback na EMA21 M30",
      "④ Confirm. (close>EMA21+EMA9)",
      "⑤ Posicao vs VWAP diario",
      "⑥ RSI M30 em zona valida",
...
void UpdatePanel(double he21,double he50,double hrsi,
                 double e9,double e21,double cl,double op,
                 double rsi30,double vwap,double score,
                 bool fL,bool fS)
{
   if(!InpPanel||!panelOk) return;

   // C1
   bool c1l=he21>he50, c1s=he21<he50;
   SetC(PFX+"V0", c1l, c1s, c1l?"▲ BULL":(c1s?"▼ BEAR":"NEUTRO"));

   // C2
   bool c2l=hrsi>=InpRSILong, c2s=hrsi<=InpRSIShort;
   SetC(PFX+"V1", c2l, c2s, StringFormat("%.1f  %s",hrsi,(c2l||c2s)?"✓":"—"));
   ...
   // C6
   bool c6l=rsi30>=InpRSILMin&&rsi30<=InpRSILMax;
   bool c6s=rsi30>=InpRSISMin&&rsi30<=InpRSISMax;
   SetC(PFX+"V5", c6l, c6s, StringFormat("%.1f  %s",rsi30,(c6l||c6s)?"✓":"—"));
